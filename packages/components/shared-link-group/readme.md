...
组件已完全适配暗黑模式，会根据系统偏好自动切换：

**亮色模式：**
- 按钮背景：`var(--background-200)`
- 按钮边框：`var(--normal-300)`
- 图标颜色：`var(--text-secondary)`
- 工具提示背景：`var(--background-100)`

**暗黑模式：**
- 按钮背景：`var(--normal-700)` - 更深的背景以确保对比度
- 按钮边框：`var(--normal-500)` - 明显的边框轮廓
- 图标颜色：`var(--text-primary)` + `opacity: 0.85` - 清晰可见
- 工具提示背景：`var(--normal-800)` - 深色背景配合浅色文字

### 对比度优化

在暗黑模式下，组件会自动调整以确保文字和图标清晰可见：
- 增强了按钮背景与边框的对比度
- 调整了图标的透明度以确保可读性
- 工具提示使用深色背景配合浅色文字，提供更好的视觉反馈
