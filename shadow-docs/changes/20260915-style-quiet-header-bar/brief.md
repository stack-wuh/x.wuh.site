...
- **追加决策（用户要求「主题入口换设计」，三选中推荐「朱砂印」）:** 纯调色板图标仍是通用 App 词汇，与墨签弹层两种世界观。入口改朱砂印「墨」——弹层样本是「试墨」、按钮是「钤印」，点开动作即盖印选墨，隐喻闭环。印面即装饰：去触发器的渐隐下划线（下划线语言留给导航与墨字段，双份装饰违背"出门前摘掉一件首饰"）。可发现性三件套：aria-label（含当前主题态）+ 原生 `title` + cursor。移动端外观行保留调色盘图标（列表行≠印）。状态实现教训：先做跨组件插值 `.trigger:hover ${ThemeSeal}` + `[aria-expanded] ${ThemeSeal}`，在 SSR 双写 styleSheets 下实测边框不生效（规则存在于 CSSOM、元素 matches 为 true、隔离新元素应用正常——插值类选择器在此环境不可靠），改 transient prop `$open` + 印面自身 `:hover`；边框色从 `border` 简写拆成长写（color-mix 进简写时态覆盖不稳）。未聚焦 webview 冻结过渡重算导致动态态无法在 IAB 内终验，已在结果段记录限制与取证链。
...
### Phase 4 · 主题入口钤印化（用户追加：图标入口"换一下设计"）

- [x] 调色盘图标入口改朱砂印「墨」：18×18 方印（印框 `color-mix(primary 45%, transparent)`、`--border-radius-xs`、印面衬线「墨」`--font-size-xs`）；去触发器渐隐下划线（印面即装饰）；行盒与 NavLink 同高保持行节奏；可发现性 aria-label + 原生 `title='外观设置'`；移动端外观行保留调色盘图标（列表行不是印）——`styles/index.ts` + `index.tsx`
- [x] 印面状态从跨组件插值选择器（`&:hover ${ThemeSeal}`/`[aria-expanded] ${ThemeSeal}`，SSR 双写 styleSheets 下实测不可靠）改为 transient prop `$open` + 自身 `:hover`；守卫同步（15 条）+ 变异验证 + tsc + 开合/悬停实测——`test/header-quiet-bar.test.mjs`

## 结果

- 实际耗时: 2026-09-15（propose → apply → 三轮用户反馈追加 → 墨签重设计 → 入口钤印化，同日完成）
- 验证: 守卫测试 15 条全绿（含 16 轮变异验证，全部"改坏必红"）；`tsc --noEmit` 干净。静默条实测：Noto Sans SC、导航 13px/平板·15px/PC 分档、行高 1.5、断点 641/700/1280 桌面态同步、375 汉堡、图标与导航下划线同 y（差 0）、logo 随档 42×26→48.5×30、移动菜单项 15px、320–1280 无横向溢出。墨签弹层实测：真实点击路径四主题轮转，纸面 `#FFFBF8`/`#111`/`#FFFDF9`/`#1C1814` 各自正确，选中下划线 opacity 1/未选中 0，桌面+移动双实例状态同步。钤印入口实测：印 18×18、衬线「墨」12px、圆角 2px、随主题换色（酒红 `rgb(201,74,68)`/素雅深 `rgb(222,184,150)`/素雅浅 `rgb(168,115,72)`）、行盒 38.5 与导航同高、弹层开合正常。**已知验证限制**：未聚焦的 IAB webview 冻结 hover/类切换的过渡重算（`seal.matches(':hover')===true` 但 computed 停起始值；rAF 超时；隔离新建同名 class 元素立即得终值——证明规则与 cascade 正确、环境不可测态切换）。验证纪律：主题路由必须走 UI 点击——setAttribute 直改会与 ThemeModeProvider 竞态出假数据；IAB 探针量"状态态切换"需页面前台，否则只可验静态渲染与规则存在。
