...
## 决策
- **选型:** 方案 A 三阶段组合——Phase 1 CJK 字体 unicode-range 切片（cn-font-split 自托管，保持 --font-sans/--font-serif 变量契约）；Phase 2 撤销详情页整页骨架（回退 ISR 命中页的流式形态，HTML 181KB→约110KB）；Phase 3 bundle 分析 + 低频重组件 next/dynamic 延迟化（分享弹窗/导出全文/评论/音频播放器等）
...
- [ ] Lighthouse 前后对比（移动模拟），记录字体传输字节变化 — 验证
...
- [ ] Lighthouse 前后对比，记录 JS 总量/文件数/FCP/TBT — 验证
