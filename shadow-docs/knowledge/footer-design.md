...
字号/行高用 `getComputedStyle` 复核（本地 dev，1280 宽）：根层与各行 12px、字体族 `Noto Sans SC`；行高 21.6px（酒红 1.8）/ 24px（素雅 2.0）；slogan `Noto Serif SC` 15px / 20.25px；tooltip 12px / 16.2px 且向上弹出。页脚高度约 273px（酒红 @1280）、281px（素雅 @1280），320–1280 宽均无横向溢出，注脚折行只发生在段边界、<520px 技术栈段整体隐藏。
