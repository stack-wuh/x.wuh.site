...
相关文章基于标签与时间排序、去重且最多 3 篇：每个标签并发请求最多 10 篇候选，按共享标签数降序、更新时间降序、编号升序排序。「继续阅读」模块以轻薄便签组呈现，使用作者口吻引导、淡纸色背景、细边框、折角与轻微错落；hover 仅轻微上浮、边框染上主题色、箭头轻微右移。窄屏下单向触达高度不低于 44px。
