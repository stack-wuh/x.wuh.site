...
export function GuestbookPage() {
  ...
  useEffect(() => {
    apiRequest<AdminCommentsResponse>('/admin/guestbook/comments?page=1&limit=20')
  ...
  return (
    <section className="page-section">
      <div className="page-heading"><h1>留言板</h1><ReadonlyHint /></div>
      {error ? <div className="page-state error">{error}</div> : (
        <DataTable data={comments} columns={[
          ...
          { key: 'actions', title: '操作', render: () => <PermissionGate fallback={<span>只读</span>}><button>处理</button></PermissionGate> },
        ]} />
      )}
    </section>
  );
